...
function resizeCanvas() {
    ...
    // Use actual window viewport dimensions to bypass browser address bars
    const viewW = window.innerWidth;
    const viewH = window.innerHeight;
    
    const isMobileLandscape = (viewW <= 1024) && (viewW > viewH);
    
    if (isMobileLandscape) {
        // Enforce safe bounds with a 12px margin (6px all around)
        const margin = 12;
        const containerW = viewW - margin;
        const containerH = viewH - margin;
        ...
        // Mobile portrait mode (let CSS warning overlay handle presentation)
        ...
        // Desktop default fixed style
        container.style.width = "960px";
        container.style.height = "540px";
        container.style.borderRadius = "16px";
        container.style.border = "1px solid rgba(255, 255, 255, 0.05)";
        container.style.boxShadow = "0 25px 50px -12px rgba(0, 0, 0, 0.7), 0 0 40px rgba(6, 182, 212, 0.15)";
    }
